...
export default function RoomsPage() {
  ...
  const [modal, setModal] = useState<{ open: boolean; editing?: Room }>({
    open: false,
  });
  ...
  const q = useQuery({
    queryKey: ["rooms"],
    queryFn: () =>
      unwrapPaginated<Room>(
        api.get("/rooms", { params: { page: 1, pageSize: 100 } }),
      ),
  });
  ...
  const save = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.push({
        title: modal.editing ? t("common.saved") : t("rooms.addRoom"),
        tone: "success",
      });
      setModal({ open: false });
    },
            ...
            <tbody>
              {q.data?.items.map((r, i) => (
                <Tr key={r.id} index={i}>
                  ...
                  <Td className="text-[13.5px] text-muted-foreground">
                    {r.building
                      ? `${r.building}${r.floor != null ? ` / F${r.floor}` : ""}`
                      : "—"}
                  </Td>
                  <Td>
                    {r.gpsEnabled ? (
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <Input
              ...
              value={form.building ?? ""}
              onChange={(e) =>
                setForm({ ...form, building: e.target.value || undefined })
              }
            />
          </div>
          <div>
          ...
          <div className="flex items-end">
            <label className="inline-flex cursor-pointer items-center gap-2 text-xs">
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, gpsEnabled: e.target.checked })
                }
              />
              GPS enforced
            </label>
