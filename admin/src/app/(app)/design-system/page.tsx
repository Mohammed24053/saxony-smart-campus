...
export default function DesignSystemPage() {
  ...
  return (
    <PageEnter>
      ...
      <Section
        title="Brand palette"
        hint="Extracted from the SEU logo. All UI utilities draw from these tokens."
      >
      ...
      <Section
        title="Buttons"
        hint="Click any button to see the 0.96-scale press animation."
      >
      ...
      <Section
        title="KPI cards"
        hint="Animated count-up + staggered entrance + optional live dot or alert badge."
      >
        <KPIRow>
          <KPICard
      ...
      <Section
        ...
        hint="Recharts area chart with brand gradient + 1000ms draw-in."
      >
        <Card>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Smooth tween from 0 → value (1.2s).
              </CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Per-digit flip on update (300ms each).
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="text-4xl font-semibold tabular-nums">
              ...
              <div className="mt-2 flex gap-2">
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => setCounter((v) => Math.max(0, v - 1))}
                >
                  −1
                </Button>
                <Button size="sm" onClick={() => setCounter((v) => v + 1)}>
                  +1
                </Button>
                <Button
                  size="sm"
                  variant="accent"
                  onClick={() => setCounter((v) => v + 10)}
                >
                  +10
                </Button>
              </div>
      ...
      <Section
        ...
        hint="Shimmer animation runs 1.5s on a loop."
      >
        <div className="grid gap-4 sm:grid-cols-2">
          <KPISkeleton />
          <Card>
      ...
      <Section
        ...
        hint="Drag-and-drop with progress + brand-coloured accept state."
      >
        <FileUploadZone
          onUpload={async () => undefined}
          hint="Excel files up to 5MB (demo)"
        />
      </Section>
      ...
      <Section
        title="Confirm modal"
        hint="Spring entrance, navy backdrop blur."
      >
      ...
      <Section
        title="Table"
        hint="Stagger fade-in (20ms per row) + hover lift."
      >
        <Table>
          <thead>
            <tr>
      ...
      <Section
        ...
        hint="Reference for all motion specs in the round-2 spec."
      >
        <Card>
          <CardContent className="pt-6">
                  ...
                  {[
                    ...
                    <Tr key={row[0]} index={i}>
                      {row.map((c, j) => (
                        <Td
                          key={j}
                          className={j === 0 ? "font-mono text-xs" : ""}
                        >
                          {c}
                        </Td>
                      ))}
...
function Section({
  ...
  return (
    ...
    >
      ...
      {hint && (
        <p className="mt-0.5 mb-3 text-sm text-muted-foreground">{hint}</p>
      )}
