...
type Subject = {
  id: string; code: string; name: string; type: 'lecture' | 'lab' | 'tutorial';
...
export default function SubjectsPage() {
  ...
  const [modal, setModal] = useState<{ open: boolean; editing?: Subject }>({ open: false });
  ...
  const save = useMutation({
    mutationFn: async (input: SubjectForm) => {
      if (modal.editing) return (await api.put(`/subjects/${modal.editing.id}`, input)).data;
      return (await api.post('/subjects', input)).data;
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['subjects'] });
      toast.push({ title: modal.editing ? t('common.saved') : t('subjects.addSubject'), tone: 'success' });
      setModal({ open: false });
    },
  ...
  const openEdit = (s: Subject) => {
    setForm({ code: s.code, name: s.name, type: s.type, faculty: s.faculty, hoursPerWeek: s.hoursPerWeek });
  ...
  return (
    <>
              ...
              {q.data?.items.map((s, i) => (
                <Tr key={s.id} index={i}>
                  ...
                  <Td className="text-right">
                    <button onClick={() => openEdit(s)} aria-label={t('common.edit')} className="mr-1 rounded p-1 text-muted-foreground hover:bg-muted hover:text-foreground">
                      <Pencil className="h-3.5 w-3.5" />
                    </button>
                    <button onClick={() => setConfirmDel(s)} aria-label={t('common.delete')} className="rounded p-1 text-muted-foreground hover:bg-seu-red/10 hover:text-seu-red">
      ...
      <Modal
        ...
        title={modal.editing ? `${t('common.edit')}: ${modal.editing.name}` : t('subjects.addSubject')}
        footer={
          <>
      ...
      >
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <Select id="type" value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value as SubjectForm['type'] })}>
          ...
          <div>
            ...
            <Input id="faculty" value={form.faculty ?? ''} onChange={(e) => setForm({ ...form, faculty: e.target.value || undefined })} />
          </div>
          <div>
            <Label htmlFor="hpw">Hours/wk</Label>
            <Input id="hpw" type="number" value={form.hoursPerWeek ?? ''} onChange={(e) => setForm({ ...form, hoursPerWeek: e.target.value ? Number(e.target.value) : undefined })} />
          </div>
          <div>
            <Label htmlFor="cap">Max capacity</Label>
            <Input id="cap" type="number" value={form.maxRoomCapacity ?? ''} onChange={(e) => setForm({ ...form, maxRoomCapacity: e.target.value ? Number(e.target.value) : undefined })} />
          </div>
        </div>
      </Modal>
