...
export default function SettingsPage() {
  ...
  const q = useQuery({
    queryKey: ["settings"],
    queryFn: async () =>
      (await api.get("/settings")).data.data ??
        ...
        <Card className="p-5">
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <Select
                id="dow"
                value={form.weekStartsOn ?? 6}
                onChange={(e) =>
                  setForm({ ...form, weekStartsOn: Number(e.target.value) })
                }
              >
                {dayLabels.map((d, i) => (
            ...
            <div>
              <Label htmlFor="late">
                {t("settings.defaultLateAfterMinutes")}
              </Label>
              <Input
                id="late"
                type="number"
                min={0}
                max={120}
                value={form.defaultLateAfterMinutes ?? 10}
                onChange={(e) =>
                  setForm({
                    ...form,
                    defaultLateAfterMinutes: Number(e.target.value),
                  })
                }
              />
            </div>
