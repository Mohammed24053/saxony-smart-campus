...
type Doctor = { id: string; doctorId: string; name: string; email?: string; phone?: string; isActive?: boolean };
...
export default function DoctorsPage() {
  ...
  const save = useMutation({
    mutationFn: async (input: DoctorForm) => {
      if (modal.editing) return (await api.put(`/doctors/${modal.editing.id}`, input)).data;
      return (await api.post('/doctors', input)).data;
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['doctors'] });
      toast.push({ title: modal.editing ? t('common.saved') : t('doctors.addDoctor'), tone: 'success' });
      setModal({ open: false });
    },
  ...
  const openEdit = (d: Doctor) => {
    setForm({ doctorId: d.doctorId, name: d.name, email: d.email, phone: d.phone });
  ...
  return (
    <>
              ...
              {q.data?.items.map((d, i) => (
                <Tr key={d.id} index={i}>
                  ...
                  <Td className="text-right">
                    <button onClick={() => openEdit(d)} aria-label={t('common.edit')} className="mr-1 rounded p-1 text-muted-foreground hover:bg-muted hover:text-foreground">
                      <Pencil className="h-3.5 w-3.5" />
                    </button>
                    <button onClick={() => setConfirmDel(d)} aria-label={t('common.delete')} className="rounded p-1 text-muted-foreground hover:bg-seu-red/10 hover:text-seu-red">
      ...
      <Modal
        ...
        title={modal.editing ? `${t('common.edit')}: ${modal.editing.name}` : t('doctors.addDoctor')}
        footer={
          <>
      ...
      >
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <Input id="docid" value={form.doctorId} onChange={(e) => setForm({ ...form, doctorId: e.target.value })} required />
          </div>
          <div>
          ...
          <div>
            ...
            <Input id="email" type="email" value={form.email ?? ''} onChange={(e) => setForm({ ...form, email: e.target.value || undefined })} />
          </div>
          <div>
          ...
          {!modal.editing && (
            <div className="col-span-2">
              ...
              <Input id="pwd" type="password" value={form.password ?? ''} onChange={(e) => setForm({ ...form, password: e.target.value || undefined })} placeholder="Optional — auto-generated if blank" />
            </div>
          )}
        </div>
      </Modal>
