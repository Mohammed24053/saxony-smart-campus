...
export default function SectionsPage() {
  ...
  const [modal, setModal] = useState<{ open: boolean; editing?: Section }>({ open: false });
  ...
  const save = useMutation({
    mutationFn: async (input: SectionForm) => {
      if (modal.editing) return (await api.put(`/sections/${modal.editing.id}`, input)).data;
      return (await api.post('/sections', input)).data;
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['sections'] });
      toast.push({ title: modal.editing ? t('common.saved') : t('sections.addSection'), tone: 'success' });
      setModal({ open: false });
    },
  ...
  const openEdit = (s: Section) => {
    setForm({ name: s.name, faculty: s.faculty, year: s.year, capacity: s.capacity });
    setModal({ open: true, editing: s });
  };

  return (
    <>
      <PageHeader title={t('sections.title')} description={t('sections.subtitle')}>
      ...
      <Modal
        ...
        title={modal.editing ? `${t('common.edit')}: ${modal.editing.name}` : t('sections.addSection')}
        footer={
          <>
      ...
      >
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <Input id="year" type="number" min={1} value={form.year ?? ''} onChange={(e) => setForm({ ...form, year: e.target.value ? Number(e.target.value) : undefined })} />
          </div>
          <div>
            <Label htmlFor="cap">Capacity</Label>
            <Input id="cap" type="number" min={1} value={form.capacity ?? ''} onChange={(e) => setForm({ ...form, capacity: e.target.value ? Number(e.target.value) : undefined })} />
          </div>
        </div>
      </Modal>
