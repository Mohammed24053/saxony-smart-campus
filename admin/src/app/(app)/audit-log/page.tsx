...
export default function AuditLogPage() {
  ...
  const q = useQuery({
    ...
    queryFn: () => unwrapPaginated<AuditEntry>(
      api.get('/audit', { params: { page, pageSize: 50, action: action || undefined } }),
    ),
  });
              ...
              {q.data?.items.map((row, i) => (
                <Tr key={row.id} index={i}>
                  ...
                  <Td className="text-[12.5px]">
                    ...
                    {row.actorRole && (
                      <span className="ml-1.5 rounded bg-muted px-1 py-px font-mono text-[10px] text-muted-foreground">
                        {row.actorRole}
                      </span>
                    )}
                  </Td>
                  ...
                  <Td className="font-mono text-[11.5px] text-muted-foreground">{row.ipAddress ?? '—'}</Td>
                </Tr>
              ))}
            </tbody>
          </table>
